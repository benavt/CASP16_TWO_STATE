...
def assessment(ID, score):
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v1_ref'], label=f'Best {score} v1 ref')
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v2_ref'], bottom=combined_df[f'best_{score}_v1_ref'], label=f'Best {score} v2 ref')
    plt.xlabel('Group')
    plt.ylabel('Score')
    plt.title(f'Aggregate Global {score} scores for {ID} V1 and V2')
    plt.legend()
