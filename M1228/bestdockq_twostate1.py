...
def assessment(ID, score):
    ...
    adjust_text(texts, arrowprops=dict(arrowstyle='->', color='red', lw=0.5),
                   ...
                   avoid_self=True)     
    plt.xlabel(f'{score} Score ( V1 reference state )', fontsize=18)
    plt.ylabel(f'{score} Score ( V2 reference state )', fontsize=18)
    plt.title(f'Scatter plot of {score} scores for {ID} V1 vs V2 reference states', fontsize=18)
