...
├── DUAL_STATE_LATEX_TABLES/ # LaTeX table files for dual-state scores
├── SINGLE_STATE_LATEX_TABLES/ # LaTeX table files for single-state scores
...
├── process_dual_state_score.py # Processes dual-state scores for specific targets
...
├── create_dual_state_latex_tables.py # Generates LaTeX tables for dual-state scores
├── create_single_state_latex_tables.py # Generates LaTeX tables for single-state scores
├── MakePlotsForManuscript.py # Generates publication-ready plots for manuscript figures
├── group_number_name_correspondance.csv  # Group number to name mapping
├── original_pipeline_by_NamitaDube_2024/ # Legacy modular pipeline for Composite Score 4 (non-functional)
