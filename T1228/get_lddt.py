...
def assessment(ID, score):
    ...
    v2_ref_df = pd.read_csv(f'./v2_1/valid_models_global_lddtv2.csv')
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v1_ref'], label=f'<lDDT> (V1A)', color='blue')
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v2_ref'], bottom=combined_df[f'best_{score}_v1_ref'], label=f'<lDDT> (V1B)', color='orange')
