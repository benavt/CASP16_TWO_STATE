...
def assessment(ID, score):
   ...
   # Set axis bounds with padding
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v1_ref'], label=f'<{score}> (V1)')
    plt.bar(combined_df['Group'], combined_df[f'best_{score}_v2_ref'], bottom=combined_df[f'best_{score}_v1_ref'], label=f'<{score}> (V2)')
