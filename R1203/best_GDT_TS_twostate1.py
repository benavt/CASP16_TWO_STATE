...
def assessment(ID, score):
    ...
    v1_ref_df = pd.read_csv(f'./GDT_TS_vs_Model_R1203v1_combined.csv')
    ...
    v2_ref_df = pd.read_csv(f'./GDT_TS_vs_Model_R1203v2_combined.csv')
    ...
    combined_df['Group'] = combined_df['Group'].str.replace('TS', '')
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_{score}_v1_ref'], label=f'<GDT_TS> (V1)')
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_{score}_v2_ref'], bottom=combined_df[f'best_{score}_v1_ref'], label=f'<GDT_TS> (V2)')
    plt.xlabel('Submission Group', fontsize=18)
    plt.ylabel('Two-StateScore', fontsize=18)
    ...
    plt.tight_layout()
    # Save the plot as an image file
    plt.savefig(f'../PLOTS/{ID}_{score}_two_state_1.png')
