...
def assessment(ID, score):
    ...
    v1_ref_df = pd.read_csv(f'./v1_1/valid_models_global_lddtv1.csv')
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'], combined_df[f'best_v1_ref'], label=f'<lDDT> (V1A)', color='blue')
    plt.bar(combined_df['Group'], combined_df[f'best_v2_ref'], bottom=combined_df[f'best_v1_ref'], label=f'<lDDT> (V1B)', color='orange')
