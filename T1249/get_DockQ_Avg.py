...
def assessment(ID, score):
    ...
    plt.scatter(combined_df['best_v1_ref'], combined_df['best_v2_ref'], c='blue', label='Submission Groups')
    ...
    texts = [plt.text(combined_df['best_v1_ref'].iloc[i], combined_df['best_v2_ref'].iloc[i], txt, fontsize=8) for i, txt in enumerate(combined_df['Group'].str.replace('TS', ''))]
    ...
    plt.title(f'Scatter plot of best {score} scores for {ID} V1 vs V2 reference states', fontsize=18)
    ...
    combined_df.to_csv(f'{ID}_DockQ_two_state.csv', index=False)
    # Create a stacked bar chart
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_v1_ref'], label=f'<DockQ> (V1A)')
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_v2_ref'], bottom=combined_df[f'best_v1_ref'], label=f'<DockQ> (V1B)')
