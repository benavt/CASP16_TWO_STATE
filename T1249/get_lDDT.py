...
def assessment(ID, score):
    ...
    ax_inset.set_xticks([0.7, 0.75, 0.8, 0.85])
    ax_inset.set_yticks([0.65, 0.7, 0.75, 0.8, 0.85])
    ax_inset.tick_params(labelsize=8)
    ax_inset.grid(True, linestyle='--', alpha=0.7)
    
    # Add rectangle in main plot to show zoomed region
    rect = plt.Rectangle((0.7, 0.65), 0.15, 0.2, fill=False, color='red', linestyle='--')
    ...
    combined_df.to_csv(f'{ID}_lDDT_two_state.csv', index=False)
