...
V1A_df = pd.read_csv('./v1_1/best_global_gdt.csv')
V1B_df = pd.read_csv('./v2/best_global_gdt.csv')

V1A_by_v1 = pd.read_csv('./v1_1/best_global_gdt_by_group_v1.csv')
V1A_by_v2 = pd.read_csv('./v1_1/best_global_gdt_by_group_v2.csv')
...
combined_df['V1A_final_fit'] = [0.0 for _ in range(len(combined_df['Group']))]
combined_df['V1B_final_fit'] = [0.0 for _ in range(len(combined_df['Group']))]

combined_df.to_csv(f'T1239_GDT_two_state_0.csv', index=False)
    ...
    if combined_df['V1A'].iloc[i] > combined_df['V1B'].iloc[i]:
        ...
        if combined_df['V1A_version'].iloc[i] == 'v2':
            ...
            if not V1B_by_v1[V1B_by_v1['Group'] == group].empty:
                ...
                combined_df.at[i, 'V1B_final_fit'] = 0
        ...
        if combined_df['V1B_version'].iloc[i] == 'v2':
            if not V1A_by_v1[V1A_by_v1['Group'] == group].empty:
                ...
                combined_df.at[i, 'V1A_final_fit'] = 0
...
combined_df['V1A_final_fit'] = combined_df['V1A_final_fit'].fillna(0)
combined_df['V1B_final_fit'] = combined_df['V1B_final_fit'].fillna(0)
...
combined_df.to_csv(f'T1239_GDT_two_state_1.csv', index=False)
...
plt.title(f'Scatter plot of best Global_GDT scores for T1239 V1A vs V1B', fontsize=16, fontweight='bold')
plt.legend()
plt.tight_layout()
# Save the plot as an image file
plt.savefig(f'T1239_Global_GDT_scatter_plot.png')
# plt.show()
plt.cla()
...
plt.title('Aggregate Global GDT_TS scores for T1239 V1A and V1B', fontsize=14, fontweight='bold')
plt.legend()
plt.xticks(rotation=90)

# Save the plot as an image file
plt.savefig('T1239_combined_GDT_TS_plot.png')