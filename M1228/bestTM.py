...
def assessment(ID, score):
    ...
    v1_ref_df = pd.read_csv(f'./Tmscore_v1.csv')
    ...
    v2_ref_df = pd.read_csv(f'./Tmscore_v2.csv')
   ...
   # Set axis bounds with padding
    ...
    plt.title(f'Scatter plot of best {score} scores \n for {ID} V1 vs V2 reference states', fontsize=18)
    ...
    plt.figure(figsize=(10, 6))
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_{score}_v1_ref'], label=f'<{score}> (V1)')
    plt.bar(combined_df['Group'].str.replace('TS', ''), combined_df[f'best_{score}_v2_ref'], bottom=combined_df[f'best_{score}_v1_ref'], label=f'<{score}> (V2)')
